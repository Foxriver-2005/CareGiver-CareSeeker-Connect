...
public class CareSeeker_Appointment_Adapter extends RecyclerView.Adapter<CareSeeker_Appointment_Adapter.ViewHolder>{

    private List<Admin_Payment_Class> payments;

    public CareSeeker_Appointment_Adapter(List<Admin_Payment_Class> payments) {
        this.payments = payments;
    }

    @NonNull
    @Override
    public CareSeeker_Appointment_Adapter.ViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        View view= LayoutInflater.from(parent.getContext()).inflate(R.layout.careseeker_appointments_rv,parent,false);
        CareSeeker_Appointment_Adapter.ViewHolder viewHolder=new CareSeeker_Appointment_Adapter.ViewHolder(view);
        return viewHolder;
    }

    @Override
    public void onBindViewHolder(@NonNull CareSeeker_Appointment_Adapter.ViewHolder holder, int position) {
        ...
        if(admin_payment_class.getStatus() == 2){
            holder.cancel.setText("Appointment Cancelled by Care Seeker!");
        ...
        if(admin_payment_class.getPayment()==1 && admin_payment_class.getStatus()==0) {
            holder.itemView.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View v) {
                    ...
                    i.putExtra("title", "Care Connect");
                    holder.itemView.getContext().startActivity(i);

                }
            });
        }
        DatabaseReference reference = FirebaseDatabase.getInstance("https://careconnect-2b4ca-default-rtdb.firebaseio.com/").getReference("CareGiver_Data");
