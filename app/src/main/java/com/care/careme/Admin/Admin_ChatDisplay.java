...
import com.care.careme.Helpers.Admin_UserAdapter;
...
public class Admin_ChatDisplay extends AppCompatActivity {
        ...
        reference_doc.addValueEventListener(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot snapshot) {
                ...
                for(DataSnapshot snapshot1: snapshot.getChildren()){
                    if((snapshot1.child("user_type").getValue(String.class)).equals("CareGiver")) {
    ...
    private void readChats(String email) {
        if(usersList.size() == 0){
            ...
            reference = FirebaseDatabase.getInstance("https://careconnect-2b4ca-default-rtdb.firebaseio.com/").getReference("CareSeeker_Details");
            reference.child(email).addValueEventListener(new ValueEventListener() {
                ...
                public void onDataChange(@NonNull DataSnapshot snapshot) {
                    ...
                    if (mUsers.size() == 0) {
                        Toast.makeText(Admin_ChatDisplay.this, "CareGiver has no Current Chats", Toast.LENGTH_SHORT).show();
                    } else {
                        adminUserAdapter = new Admin_UserAdapter(Admin_ChatDisplay.this, mUsers, email);
                        recyclerView.setAdapter(adminUserAdapter);
                    }
